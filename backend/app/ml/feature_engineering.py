...
def batch_features(
    ...
    if cust_df is not None:
        cust_cols = ["customer_id", "risk_score", "customer_tier", "balance_amount",
                     "account_opening_date", "kyc_level", "profile_type",
                     "card_network", "card_status"]
        cust_cols = [c for c in cust_cols if c in cust_df.columns]
        df = df.merge(cust_df[cust_cols].rename(columns={
    ...
    df["feat_is_night"]      = ts.dt.hour.between(22, 23).astype(int) | ts.dt.hour.between(0, 5).astype(int)
    ...
    df["feat_txn_type"]       = df.get("transaction_type", pd.Series(["purchase"] * len(df))).map(txn_type_map).fillna(0).astype(int)
    ...
    df["feat_customer_tier"]  = df.get("c_tier", pd.Series(["standard"] * len(df))).map(tier_map).fillna(0)

    df["feat_balance"]        = df.get("c_balance", pd.Series([50000.0] * len(df))).fillna(50000).astype(float)
    df["feat_amount_to_balance"] = np.where(
        df["feat_balance"] > 0,
        df["feat_amount"] / df["feat_balance"],
        1.0
    )
    ...
    if "c_opening_date" in df.columns:
        df["feat_account_age_days"] = (now - pd.to_datetime(df["c_opening_date"], errors="coerce", utc=True)).dt.days.fillna(365)
    else:
        df["feat_account_age_days"] = 365

    kyc_map = {"basic": 0, "enhanced": 1, "full": 2}
    df["feat_kyc_level"]      = df.get("c_kyc_level", pd.Series(["basic"] * len(df))).map(kyc_map).fillna(0)

    df["feat_is_compromised_profile"] = (df.get("c_profile_type", pd.Series([""] * len(df))) == "compromised").astype(int)
    ...
    df["feat_is_cross_border_high_amount"] = (df["feat_is_foreign"] & (df["feat_amount"] > 10000)).astype(int)
    df["feat_night_high_amount"] = (df["feat_is_night"] & (df["feat_amount"] > 5000)).astype(int)
    df["feat_online_high_amount"] = (df["feat_is_online"] & (df["feat_amount_zscore"] > 2)).astype(int)
    df["feat_new_account_high_txn"] = ((df["feat_account_age_days"] < 90) & (df["feat_amount"] > 10000)).astype(int)
    df["feat_velocity_high_amount"] = (df["feat_txn_count_1h"] > 3).astype(int) * (df["feat_amount_zscore"] > 1).astype(int)
