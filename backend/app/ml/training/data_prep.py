...
def load_data(
    ...
    if not os.path.exists(cust_path):
        raise FileNotFoundError(f"Customers CSV not found: {cust_path}\n  Run: python scripts/seed_data.py")
    ...
    # Parse timestamps
    txn_df["transaction_timestamp"] = pd.to_datetime(txn_df["transaction_timestamp"], utc=True, errors="coerce")
    ...
    print(f"  Labels — Fraud: {fraud_count} ({fraud_count/len(txn_df)*100:.1f}%)  Legit: {legit_count}")
