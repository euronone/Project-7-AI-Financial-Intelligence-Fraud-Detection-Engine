...
class Tenant(Base):
    ...
    institution_type: Mapped[str] = mapped_column(String(30), default="bank")  # bank|fintech|insurance|payment_processor|neobank
    ...
    db_type: Mapped[str | None] = mapped_column(String(30), nullable=True)  # supabase|postgresql|mysql|mongodb|rest_api
    ...
    # ML model settings
    model_strategy: Mapped[str] = mapped_column(String(20), default="shared_global")  # shared_global|dedicated|hybrid
...
class User(Base):
    ...
    must_change_password: Mapped[bool] = mapped_column(Boolean, default=False)  # Admin-forced reset flag
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))
