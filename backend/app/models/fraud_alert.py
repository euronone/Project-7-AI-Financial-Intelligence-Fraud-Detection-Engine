...
class FraudAlert(Base):
    ...
    alert_type: Mapped[str] = mapped_column(String(20), default="ml_model")  # rule|ml_model|manual|watchlist
    severity: Mapped[str] = mapped_column(String(20), default="medium", index=True)  # low|medium|high|critical
    status: Mapped[str] = mapped_column(String(20), default="open", index=True)  # open|under_review|confirmed_fraud|false_positive|closed

    # Review
    analyst_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("users.id"), nullable=True)
