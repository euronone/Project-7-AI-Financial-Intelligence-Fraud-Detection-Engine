  ...
  const navContent = (
    <>
      ...
      <div className="flex h-16 items-center gap-3 border-b border-slate-200/80 px-4">
        ...
        {!sidebarCollapsed && <span className="text-lg font-bold text-gray-900">FinShield</span>}
      </div>
              ...
              {group.items.map((item) => {
                ...
                return (
                  <li key={item.href}>
                    ...
                    >
                      <span className={cn(active && "text-primary-600")}>{item.icon}</span>
                      {!sidebarCollapsed && <span>{item.label}</span>}
                      {!sidebarCollapsed && item.badge && (
                        <span className="ml-auto rounded-full bg-danger-100 px-2 py-0.5 text-[10px] font-semibold text-danger-700">
                          {item.badge}
                        </span>
                      )}
                    </Link>
    ...
    <>
      ...
      {sidebarMobileOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <aside className="fixed left-0 top-0 z-50 flex h-screen w-64 flex-col border-r bg-white shadow-xl animate-slide-in-right">
            {navContent}
          </aside>
        </div>
      )}
