...
## Team Task List (8 People) — Merge-Safe Plan

### Working Agreement (to avoid merge conflicts)
- **One feature per branch**: `feature/<area>-<short-name>` (example: `feature/backend-transactions-crud`)
- **No overlapping file ownership**: each person owns a defined set of folders/files below.
- **PR requirements**: unit tests added/updated, CI passing, no secrets, follows repo conventions.
- **Integration point**: all PRs target `main` and should be small enough to review (<500 LOC when possible).

### Global Definition of Done (applies to everyone)
- **Code**: feature implemented in the correct layer (API → service → repo/model when applicable)
- **Validation & security**: input validation, auth/permission checks where needed, no sensitive logs
- **Tests**: at least basic unit tests for non-trivial logic; integration tests for cross-boundary flows
- **Docs**: update `docs/API_SPEC.md` / `docs/DB_SCHEMA.md` only if your task changes them

---

## Person 1 — Backend Foundation: App bootstrap + common patterns
- **Goal**: ensure FastAPI app skeleton is clean and consistent for all other backend tasks.
- **Primary ownership**:
  - `backend/app/main.py`
...
- **Deliverables**:
  - Standardized error response shape (no internal stack traces to clients)
  - Request correlation ID + structured logging baseline (minimal, production-safe)
  - CORS configuration via settings/env
- **Tests**:
  - `backend/tests/integration/test_api_endpoints.py` for health + basic error handler behavior

## Person 2 — Backend Auth & RBAC (JWT + permissions)
- **Goal**: implement secure auth flows and RBAC gates that other endpoints can reuse.
- **Primary ownership**:
...
- **Deliverables**:
  - Login + `/auth/me`
  - RBAC dependency helpers (admin/analyst/investigator/viewer)
  - Password policy + lockout (as per PRD) where feasible
- **Tests**:
  - `backend/tests/unit/test_auth.py`
  - A small integration flow in `backend/tests/integration/test_api_endpoints.py`

## Person 3 — Backend Transactions API (CRUD + list filters)
- **Goal**: build the core transaction endpoints (foundation for fraud pipeline later).
- **Primary ownership**:
...
- **Deliverables**:
  - GET `/transactions` with pagination + filters (status, risk_level, amount range, date range)
  - GET `/transactions/{id}` basic detail (fraud analysis can be stubbed if not ready)
  - POST `/transactions` ingestion (writes transaction record; pipeline hook can be a TODO event)
- **Tests**:
  - `backend/tests/integration/test_transaction_flow.py` basic list/create/get

## Person 4 — Backend Rules Engine (evaluation core + CRUD endpoints)
- **Goal**: implement rules engine service and API so transactions can be evaluated.
- **Primary ownership**:
  - `backend/app/rules/engine.py`
  - `backend/app/rules/conditions.py`
  - `backend/app/rules/actions.py`
  - `backend/app/api/v1/rules.py`
  - `backend/app/services/rules_engine_service.py`
  - `backend/app/models/rule.py`
  - `backend/app/schemas/rule.py`
- **Deliverables**:
  - Rule CRUD + toggle
  - Rule evaluation function that takes a transaction payload and returns matched rules + actions
- **Tests**:
  - `backend/tests/unit/test_rules_engine.py` for condition/operator behavior

## Person 5 — Backend Fraud Alerts + Cases workflow (core CRUD + status transitions)
- **Goal**: implement the operational workflow (alerts and cases) used by the UI.
- **Primary ownership**:
  - `backend/app/api/v1/fraud_alerts.py`
  - `backend/app/api/v1/cases.py`
  - `backend/app/services/*(fraud_alert|case)_service.py`
  - `backend/app/models/fraud_alert.py`
  - `backend/app/models/case.py`
  - `backend/app/schemas/fraud_alert.py`
  - `backend/app/schemas/case.py`
- **Deliverables**:
  - Alert list/detail endpoints
  - Update status/assign/resolve flows (enforce allowed transitions)
  - Create case from alert endpoint
- **Tests**:
  - `backend/tests/integration/test_alert_workflow.py`
  - `backend/tests/integration/test_case_management.py`

## Person 6 — Frontend Shell + Transactions UI (pages + components)
- **Goal**: build the user-facing transaction list and detail pages with clean UX.
- **Primary ownership**:
  - `frontend/src/app/(dashboard)/transactions/**`
...
- **Deliverables**:
  ...
  - Transaction detail page layout (sections for raw data, risk/fraud summary, linked alerts)
  - API client calls via existing `frontend/src/lib/api-client.ts`
- **Tests**:
  - `frontend/tests/unit/components/` basic component tests for filters/table

## Person 7 — Frontend Alerts + Cases UI (workflow screens)
- **Goal**: enable analysts to manage alerts and cases end-to-end from the dashboard.
- **Primary ownership**:
  - `frontend/src/app/(dashboard)/fraud-alerts/**`
  - `frontend/src/app/(dashboard)/case-management/**`
  - `frontend/src/components/fraud/**`
  - `frontend/src/hooks/use-alerts.ts`
  - `frontend/src/types/alert.ts`
  - `frontend/src/types/case.ts`
- **Deliverables**:
  - Alerts list + detail pages with status/assign/resolve actions
  - Case list + case detail timeline UI (can start simple; timeline entries render reliably)
- **Tests**:
  - Add at least 1 Playwright smoke test: open alerts list and view a detail page

## Person 8 — Infrastructure & DevX (docker-compose + CI + docs touch-ups)
- **Goal**: make the repo easy to run locally and ensure CI guards quality.
- **Primary ownership**:
  - `docker-compose.yml`
  ...
  - `README.md`
  - `docs/DEPLOYMENT.md`
  - `backend/.env.example` + `frontend/.env.local.example` (examples only; no real secrets)
- **Deliverables**:
  - Local dev flow documented and working (docker up + backend + frontend)
  - CI runs lint/test/build for frontend + backend (as defined in PRD)
  - Ensure `.gitignore` covers common local artifacts (env files, caches)

---

## Merge Playbook (for you as lead)
- **Daily**: each person opens a PR to `main`, tagged with their area (backend/frontend/infra).
- **Order (recommended)**:
  - Person 1 → Person 2 → Persons 3/4/5 (parallel) → Persons 6/7 (parallel) → Person 8 (continuous)
- **Conflict prevention**:
  - Don’t let multiple people edit the same shared file (especially `backend/app/main.py`, `frontend/src/lib/api-client.ts`, workflow yaml) unless explicitly coordinated.
